...
// ✅ Get user's cart
router.get("/", auth, requireRole(['customer']), async (req, res, next) => {
  try {
...
// ✅ Add to cart (same as before - no change)
...
router.put("/item/:menuItemId", auth, requireRole(['customer']), async (req, res, next) => {
  try {
    const userId = req.user._id || req.user.userId;
...
router.delete("/item/:menuItemId", auth, requireRole(['customer']), async (req, res, next) => {
  try {
    const userId = req.user._id || req.user.userId;
    const { menuItemId } = req.params;
...
// ✅ Clear entire cart
router.delete("/", auth, requireRole(['customer']), async (req, res, next) => {
  try {
    const userId = req.user._id || req.user.userId;
...
// ✅✅✅ FIXED: Apply coupon code (NO usedCount increment here)
router.post("/apply-coupon", auth, requireRole(['customer']), async (req, res, next) => {
  try {
    const userId = req.user._id || req.user.userId;
    ...
    // ✅ FIXED: Database se coupon find karo
    ...
    // ✅ Store coupon in cart (NO usedCount increment)
    cart.coupon = {
      code: coupon.code,
      discount: coupon.discountValue,
      type: coupon.discountType,
      discountAmount: discountAmount,
      couponId: coupon._id  // ✅ Store coupon ID for reference
    };
...
// ✅ Remove coupon
router.delete("/coupon", auth, requireRole(['customer']), async (req, res, next) => {
  try {
    const userId = req.user._id || req.user.userId;
